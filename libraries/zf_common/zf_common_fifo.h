 ...
 *       后续读写操作会安全返回 ZF_INVALID_PARAM 或 0。
 */
void fifo_init(zf_fifo_t *fifo, uint8_t *buffer, uint32_t capacity);
 ...
 *       不得与中断中的读写操作并发执行。
 */
void fifo_clear(zf_fifo_t *fifo);
 ...
 */
uint32_t fifo_used(const zf_fifo_t *fifo);
 ...
 */
uint32_t fifo_free(const zf_fifo_t *fifo);

/**
 * @brief 向 FIFO 写入一个字节。
 * @param fifo FIFO 对象指针。
 * @param data 待写入的字节。
 * @return ZF_OK 表示成功；ZF_INVALID_PARAM 表示对象无效；
 *         ZF_ERROR 表示缓冲区已满。
 */
zf_status_t fifo_write_byte(zf_fifo_t *fifo, uint8_t data);

/**
 * @brief 从 FIFO 读取一个字节。
 * @param fifo FIFO 对象指针。
 * @param data 读取结果输出指针。
 * @return ZF_OK 表示成功；ZF_INVALID_PARAM 表示对象或输出指针无效；
 *         ZF_ERROR 表示缓冲区为空。
 */
zf_status_t fifo_read_byte(zf_fifo_t *fifo, uint8_t *data);
 ...
 */
uint32_t fifo_write_buffer(zf_fifo_t *fifo, const uint8_t *data, uint32_t length);
 ...
 */
uint32_t fifo_read_buffer(zf_fifo_t *fifo, uint8_t *data, uint32_t length);

#ifdef __cplusplus
}
#endif

#endif /* _zf_common_fifo_h_ */
