#ifndef _rp_driver_uart_h_
#define _rp_driver_uart_h_
 ...
 * @param length 期望字节数。
 * @param timeout_ms 超时时间，单位毫秒。
 * @return RP_OK 表示读满 length 字节；RP_TIMEOUT 表示超时，已读走的字节不会回退。
 */
 ...
 * @param port UART 逻辑编号。
 * @param byte 接收字节输出。
 * @return RP_OK 表示取到数据，RP_ERROR 表示当前无数据。
 */
