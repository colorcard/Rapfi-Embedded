 ...
 * @return RP_OK 表示找到合法时序，其他值表示失败。
 */
static rp_status_t can_compute_timing(uint32_t clock_hz, uint32_t bitrate,
...
rp_status_t can_init(can_index_enum bus, const can_cfg_t *cfg)
{
  ...
  if (can_compute_timing(HAL_RCC_GetPCLK1Freq(), nominal_bitrate,
                         sample_permille, &prescaler, &seg1, &seg2) != RP_OK) {
    return RP_INVALID_PARAM;
  }
...
rp_status_t can_start(can_index_enum bus)
{
  ...
  return rp_from_hal(HAL_FDCAN_Start(handle));
}

rp_status_t can_stop(can_index_enum bus)
{
  FDCAN_HandleTypeDef *handle = can_handle(bus);
  ...
  return rp_from_hal(HAL_FDCAN_Stop(handle));
}

rp_status_t can_send(can_index_enum bus, const can_message_t *message,
                     uint32_t timeout_ms)
{
  FDCAN_HandleTypeDef *handle = can_handle(bus);
...
rp_status_t can_receive(can_index_enum bus, can_message_t *message,
                        uint32_t timeout_ms)
{
  FDCAN_HandleTypeDef *handle = can_handle(bus);
