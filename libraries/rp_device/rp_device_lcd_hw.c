...
#error "LCD requires BSP_ENABLE_SPI1=1 in rp_common_bsp_config.h"
#endif
...
static HAL_StatusTypeDef lcd_write(uint8_t is_data, const uint8_t *data, uint16_t size)
{
  ...
  status = (spi_write_8bit_array(LCD_SPI_INDEX, data, size, LCD_TIMEOUT_MS) == RP_OK)
               ? HAL_OK : HAL_ERROR;
  lcd_cs(GPIO_PIN_SET);
  return status;
}
