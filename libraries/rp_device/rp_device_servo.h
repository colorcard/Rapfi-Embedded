#ifndef _rp_device_servo_h_
#define _rp_device_servo_h_
 ...
 */
rp_status_t servo_init(void);

/**
 * @brief 设置舵机脉宽。
 * @param servo 舵机编号。
 * @param pulse_us 脉宽，单位微秒；超出 500~2500 时钳位到边界。
 * @return RP_OK 表示成功，RP_INVALID_PARAM 表示舵机编号非法。
 */
rp_status_t servo_set_pulse_us(servo_index_enum servo, uint16_t pulse_us);
 ...
 * @return RP_OK 表示成功，RP_INVALID_PARAM 表示舵机编号非法。
 */
rp_status_t servo_set_angle(servo_index_enum servo, int16_t angle_deg);

#ifdef __cplusplus
}
#endif

#endif /* _rp_device_servo_h_ */
