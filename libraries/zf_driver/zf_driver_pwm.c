 ...
 */
static zf_status_t pwm_ensure_timer(pwm_channel_enum channel)
{
  const pwm_hw_t *hw;
  ...
  if (!s_timer_ready[hw->timer]) {
    if (timer_hw_init(hw->timer) != ZF_OK) {
      return ZF_ERROR;
    }
...
zf_status_t pwm_init(pwm_channel_enum channel, const pwm_cfg_t *cfg)
{
  ...
  if (pwm_set_frequency(channel, frequency) != ZF_OK) {
    return ZF_INVALID_PARAM;
  }
  if (pwm_set_pulse(channel, pulse) != ZF_OK) {
    return ZF_INVALID_PARAM;
  }
  if (start) {
...
zf_status_t pwm_set_frequency(pwm_channel_enum channel, uint32_t frequency_hz)
{
  const pwm_hw_t *hw;
...
zf_status_t pwm_set_duty(pwm_channel_enum channel, uint32_t duty_permille)
{
  const pwm_hw_t *hw;
  TIM_HandleTypeDef *htim;
  uint32_t period;
...
zf_status_t pwm_set_pulse_us(pwm_channel_enum channel, uint32_t pulse_us)
{
  const pwm_hw_t *hw;
  TIM_HandleTypeDef *htim;
  uint32_t timer_clock;
...
zf_status_t pwm_start(pwm_channel_enum channel)
{
  const pwm_hw_t *hw;
